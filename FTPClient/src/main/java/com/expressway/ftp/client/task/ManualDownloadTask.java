 ...
 */
@Component
public class ManualDownloadTask {
	/** ETC稽查数据接口 */
	 ...
	 */
	public void process(String beginDate, String endDate) throws InterruptedException {
		log.info("Execute at : " + GregorianCalendar.getInstance().getTime());

		runEtcLoader(beginDate, endDate);
		runMtcLoader(beginDate, endDate);
	}
	 ...
	 */
	@Async
	private void runEtcLoader(String beginDate, String endDate) {
		etcDownloader.download(analyticalMapper.getRecords(beginDate, endDate));
	}
	 ...
	 */
	@Async
	private void runMtcLoader(String beginDate, String endDate) {
		for (IMtcMapper mapper : new IMtcMapper[] { changecardMapper, enterthecarMapper, freenotfreeMapper,
			...
			mtcDownloader.download(mapper.getRecords(beginDate, endDate), mapper);
		}
	}
}
